...
public class Main {
    ...
    private static String generarCodigoArduino(java.util.List<domotica.ast.ASTNode> ast) {
        ...
        sb.append(" \n");
        sb.append("    // Configurar termostato (LED indicador en pin 7)\n");
        ...
        for (ASTNode node : ast) {
            ...
            else if (node instanceof domotica.ast.ThermostatOnCommand) {
            sb.append("    digitalWrite(7, HIGH);  // Encender termostato\n");
            sb.append("    Serial.println(\"Termostato encendido\");\n");

            // NUEVO: TERMOSTATO OFF
            } else if (node instanceof domotica.ast.ThermostatOffCommand) {
                sb.append("    digitalWrite(7, LOW);  // Apagar termostato\n");
                sb.append("    Serial.println(\"Termostato apagado\");\n");

            // NUEVO: SET TEMPERATURA
            } else if (node instanceof domotica.ast.ThermostatSetTemperatureCommand) {
                int temp = ((domotica.ast.ThermostatSetTemperatureCommand)node).getTemperature();
                sb.append("    thermostatTemp = ").append(temp).append(";  // Ajustar temperatura\n");
                sb.append("    Serial.print(\"Temperatura ajustada a: \"); Serial.println(thermostatTemp);\n");
            }
        }
